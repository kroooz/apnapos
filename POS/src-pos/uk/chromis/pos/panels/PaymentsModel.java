...
public class PaymentsModel {
    ...
    public static PaymentsModel loadInstance(AppView app) throws BasicException {
        ...
        Double openingCash = (double)0;
        try
        {
            String openingCashString = (String) new StaticSentence(app.getSession(), "SELECT OPENINGCASH FROM CLOSEDCASH WHERE MONEY = ? "
                , SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE).find(app.getActiveCashIndex());
            openingCash = Double.parseDouble(openingCashString);
        }
        catch(Exception ex)
        {
        }
        p.m_openingCash = openingCash;
...
//        return m_sHost;
    ...
    public String printOpeningCash() {
        
        return Double.toString(m_openingCash);
    }
