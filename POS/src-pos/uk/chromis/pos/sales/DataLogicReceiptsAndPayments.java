 ...
 * @author adrianromero
 */
public class DataLogicReceiptsAndPayments extends BeanFactoryDataSingle {
     ...
     */
    public DataLogicReceiptsAndPayments() {
    }
    ...
    notes: Description about payment or receipt
    */
    public void makePaymentOrReceiptEntry(String paymentThrough, double amount, String transactionId, String partyId, String partyType, String notes, Date date) throws Exception {
        
        // validation
        if(partyType != null && !partyType.equals(AppLocal.customerTypeString) && !partyType.equals(AppLocal.supplierTypeString)) {
            throw new Exception("Party type is not valid");
        }
        
        // open cash id
        String openCashId = (String)new PreparedSentence(s,
                "SELECT MONEY FROM CLOSEDCASH WHERE DATEEND IS NULL",
                null, SerializerReadString.INSTANCE).find();
        
        if(openCashId == null || openCashId.equals("")) {
            throw new Exception("Unable to get open cash id");
        }
        
        String receiptId = UUID.randomUUID().toString();
        new StaticSentence(s, "INSERT INTO RECEIPTS ( ID, MONEY, DATENEW ) "
                + "VALUES (?, ?, ?)", new SerializerWriteBasic(new Datas[]{
                Datas.STRING, Datas.STRING, Datas.TIMESTAMP}))
                .exec( receiptId, openCashId, date );
        
        String paymentId = UUID.randomUUID().toString();
        new StaticSentence(s, "INSERT INTO PAYMENTS ( ID, RECEIPT, PAYMENT, TOTAL, TRANSID, NOTES ) "
                + "VALUES (?, ?, ?, ?, ?, ?)", new SerializerWriteBasic(new Datas[]{
                Datas.STRING, Datas.STRING, Datas.STRING, Datas.DOUBLE, Datas.STRING, Datas.STRING}))
                .exec( paymentId, receiptId, paymentThrough, amount, transactionId, notes );
